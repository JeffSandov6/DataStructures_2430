...
weight=(0  10 10 10 10 10 10 10 10 10 10)
...
grade=$baseGrade
for casenum in `seq 1 1 $casecount`; do
    echo "./$filename \"script=input.script$casenum\""
    #timeout=${tout[$casenum]}
    timeout -k $timeout $timeout ./$filename "script=input.script$casenum" 1>$casenum.stdout 2>$casenum.stderr
    if [ $? -ne 0 ]; then
        echo -e "        ${red}Program killed due to timeout ($timeout).$nocolor"
        echo "Test case $casenum timed out ($timeout)." >> TIMEOUT
    fi
    cat graph.txt | sort > sorted$casenum.out
    cat $casenum.ans    | sort > sorted$casenum.ans
    diff -iEBwu sorted$casenum.out sorted$casenum.ans > $casenum.diff
    if [ $? -ne 0 ]; then
